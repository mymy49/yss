...
drv::I2c i2c1(I2C1, setI2c1ClockEn, setI2c1IntEn, YSS_DMA_MAP_I2C1_TX_STREAM, YSS_DMA_MAP_I2C1_RX_STREAM, define::dma::priorityLevel::LOW);
...
drv::I2c i2c2(I2C2, setI2c2ClockEn, setI2c2IntEn, YSS_DMA_MAP_I2C2_TX_STREAM, YSS_DMA_MAP_I2C2_RX_STREAM, define::dma::priorityLevel::LOW);
...
static void setI2c3IntEn(bool en)
{
    nvic.setI2c3En(en);
}

drv::I2c i2c3(I2C3, setI2c3ClockEn, setI2c3IntEn, YSS_DMA_MAP_I2C3_TX_STREAM, YSS_DMA_MAP_I2C3_RX_STREAM, define::dma::priorityLevel::LOW);
#endif
...
bool I2c::send(unsigned char addr, void *src, unsigned int size, unsigned int timeout)
{
    ...
    mSize = size;
    mState = STATUS::START;
    mData = (unsigned char *)src;
    mPeri->CR2 |= I2C_CR2_ITEVTEN_Msk;

    mTimelapse.reset();
    mPeri->CR1 |= I2C_CR1_START_Msk;
    while (mState != STATUS::COMPLETE)
    {
        thread::yield();

        if (mTimelapse.getMsec() > timeout)
        {
            mPeri->CR2 &= ~(I2C_CR2_ITEVTEN_Msk | I2C_CR2_ITBUFEN_Msk | I2C_CR2_ITERREN_Msk);
            return false;
        }
    }
    mPeri->DR;
...
bool I2c::receive(unsigned char addr, void *des, unsigned long size, unsigned long timeout)
{
    ...
    mSize = size;
    mState = STATUS::START;
    mData = (unsigned char *)des;

    if (size > 1)
        mPeri->CR1 |= I2C_CR1_ACK_Msk;
    else
        mPeri->CR1 &= ~I2C_CR1_ACK_Msk;
    mPeri->CR2 |= I2C_CR2_ITEVTEN_Msk;
...
void I2c::isrEv(void)
{
    if (mPeri->SR1 & I2C_SR1_SB_Msk)
    {
        mState = STATUS::ADDR;
        mPeri->DR = mAddr;
    }

    if (mPeri->SR1 & I2C_SR1_ADDR_Msk)
    {
        mState = STATUS::DATA;
        mPeri->CR2 |= I2C_CR2_ITBUFEN_Msk;
        mPeri->SR2;
    }

    if (mState == STATUS::DATA && mAddr & 0x01 && mSize && mPeri->SR1 & I2C_SR1_RXNE_Msk)
    {
        if (mSize == 2)
            mPeri->CR1 &= ~I2C_CR1_ACK_Msk;
        *mData++ = (unsigned char)mPeri->DR;
        mSize--;
    }
    else if (mState == STATUS::DATA && mSize && mPeri->SR1 & I2C_SR1_TXE_Msk)
    {
        mPeri->DR = *mData++;
        mSize--;
    }
    ...
    if (mSize == 0)
    {
        mState = STATUS::WAIT_COMPLETE;
        mPeri->CR2 &= ~I2C_CR2_ITBUFEN_Msk;
    }

    if (mState == STATUS::WAIT_COMPLETE && mPeri->SR1 & I2C_SR1_BTF_Msk)
    {
