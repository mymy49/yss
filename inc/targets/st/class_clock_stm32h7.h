...
public:
	/* 
		...
		@ useBypass : 입력이 크리스탈일 경우에는 false로 설정합니다. 입력이 오실레이터일 경우나 클럭 소스를 직접 입력 받을 경우 bypass를 true로 설정합니다.
	*/
	...
	*/
	...
	/*
		...
		@ return : 에러를 반환합니다.
		.
		@ m : PLL 입력 클럭의 분주비를 설정합니다. 설정 가능한 범위는 0 ~ 63입니다. 만약 0일 경우 prescaler가 비활성화 되고, 그 외의 경우는 clk / m 이 됩니다.
		@ n : PLL 클럭의 증폭 배율을 설정합니다. 설정 가능한 범위는 3 ~ 511입니다. 
		@ pDiv (0 ~ 127) : PLL_P 클럭의 출력 분주비를 설정합니다. pll_p = f_src / m * (n + 1) / (pDiv + 1)
		@ qDiv (0 ~ 127) : PLL_Q 클럭의 출력 분주비를 설정합니다. pll_q = f_src / m * (n + 1) / (qDiv + 1)
		@ rDiv (0 ~ 127 / !2) : PLL_R 클럭의 출력 분주비를 설정합니다. 2는 사용불가능 합니다. pll_r = f_src / m * (n + 1) / (rDiv + 1)
	*/
