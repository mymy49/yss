...
bool Clock::setSysclk(uint8_t sysclkSrc, uint8_t ahb, uint8_t apb1, uint8_t apb2, uint8_t vcc)
{
	...
	//	// ADC 프리스케일러 설정
	//	setFieldData(peri[RCC_REG::CFGR], RCC_CFGR2_PREDIV_Msk, buf, RCC_CFGR2_PREDIV_Pos);
	//}
	
	// 버스 클럭 프리스케일러 설정
	setTwoFieldData(peri[RCC_REG::CFGR], RCC_CFGR_PPRE_Msk, apb1, RCC_CFGR_PPRE_Pos, RCC_CFGR_HPRE_Msk, ahb, RCC_CFGR_HPRE_Pos);
...
{
	return gHseFreq / (((RCC[RCC_REG::CFGR] & RCC_CFGR_PLLXTPRE_Msk) >> RCC_CFGR_PLLXTPRE_Pos) + 1) * (((RCC[RCC_REG::CFGR] & RCC_CFGR_PLLMUL_Msk) >> RCC_CFGR_PLLMUL_Pos) + 2); 
}
...
{
	return getSystemClockFrequency() / gPpreDiv[((RCC[RCC_REG::CFGR] & RCC_CFGR_PPRE_Msk) >> RCC_CFGR_PPRE_Pos)];
}
