...
static threadId_t gCurrentThreadNum[YSS__CORE_COUNT] = {
    0, 1}; // Currently executing thread
#endif
...
threadId_t add(void (*func)(void *var), void *var, int32_t stackSize,
               bool signalLock) __attribute__((optimize("-O1")));
threadId_t add(void (*func)(void *var), void *var, int32_t stackSize,
               bool signalLock) {
  uint32_t i, *sp;
  ...
  if (gNumOfThread >= MAX_THREAD) {
    ...
    debug_printf("Thread creation failed! The number of threads exceeded the "
                 "configured limit of %d.",
                 MAX_THREAD);
#endif
    return -1;
  }
...
// Create a thread and preload additional registers r8-r12 before the first
// context switch.
threadId_t add(void (*func)(void *), void *var, int32_t stackSize, void *r8,
               void *r9, void *r10, void *r11, void *r12, bool signalLock)
...
threadId_t add(void (*func)(void), int32_t stackSize, void *r8, void *r9,
               void *r10, void *r11, void *r12, bool signalLock)
    __attribute__((optimize("-O1")));
threadId_t add(void (*func)(void), int32_t stackSize, void *r8, void *r9,
               void *r10, void *r11, void *r12, bool signalLock) {
...
void yield(void) {
    ...
    defined(YSS__CORE_CM33_H_GENERIC) || defined(YSS__CORE_CM0_H_GENERIC)
  SCB->ICSR |= SCB_ICSR_PENDSVSET_Msk;
#endif
}
}
...
// Create a trigger task. Trigger tasks are activated explicitly via
// trigger::run().
...
// Disable the currently running trigger, preventing it from running until
// re-triggered.
...
#if defined(YSS__CORE_CM3_CM4_CM7_H_GENERIC) ||                                \
    defined(YSS__CORE_CM33_H_GENERIC) || defined(YSS__CORE_CM0_H_GENERIC)
  // Do not disable interrupts here to reduce latency for higher-priority
  // interrupts.
  SCB->ICSR |= SCB_ICSR_PENDSVSET_Msk;
#endif
#endif
...
/**
 * @brief PendSV exception handler for Core 0.
 * @details Performs naked assembly-level context switching for Core 0 by
 * saving FPU and standard register states onto its current process stack
 * (PSP), selecting the next runnable thread (via round-robin or
 * signal/trigger queue), and restoring the next thread's context before
 * returning.
 */
...
void PendSV_Handler(void) {
#if !defined(YSS__MCU_SMALL_SRAM_NO_SCHEDULE)
#if defined(YSS__CORE_CM3_CM4_CM7_H_GENERIC) ||                                \
    defined(YSS__CORE_CM33_H_GENERIC)
  // Move the stack pointer stored in PSP to R0
  asm("mrs r0, psp");

#if (!defined(__NO_FPU) || defined(__FPU_PRESENT)) && !defined(__SOFTFP__) ||  \
    ((__FPU_PRESENT == 1) && (__FPU_USED == 1))
  // Back up FPU-related registers
  asm("vstmdb r0!,{s16-s31}");
  asm("mov r3, lr");
  asm("stmdb r0!, {r3-r11}");
#else
  // Copy LR to R3 and back up R3 to R11 onto the stack
  asm("mov r3, lr");
  asm("stmdb r0!, {r3-r11}");
#endif
  ...
  __disable_irq();
  if (gPendingSignalThreadCount) { // Enter if a signal or trigger has occurred
    gPendingSignalThreadCount--;
    gCurrentThreadNum[cid] =
        gPendingSignalThreadList[gPendingSignalThreadCount];
    ...
    gHoldingThreadNum = -1;
    sp = (uint32_t)gYssThreadList[gCurrentThreadNum[cid]].sp;
  } else { // If there is no stack pointer update from a signal or trigger,
           // execute the thread selected by the round-robin scheduler
    ...
    do {
      ...
      if (gRoundRobinThreadNum >= MAX_THREAD)
        gRoundRobinThreadNum = 0;
#if YSS__CORE_COUNT == 2
    } while (gRoundRobinThreadNum == gCurrentThreadNum[0] ||
             gRoundRobinThreadNum == gCurrentThreadNum[1] ||
             !gYssThreadList[gRoundRobinThreadNum].able);
#endif
  ...
  // Since SysTick overflowed before context switching started,
  // the counter might have decremented if an interrupt occurred in between, so
  // reset it.
  asm("ldr r3, =0xe000e010");
  asm("movs r1, #0");
  asm("str r1, [r3, #8]");

  // Restore FPU-related registers
  asm("ldm  r0!, {r3-r11}");
  asm("vldm r0!,{s16-s31}");
  asm("mov lr, r3");
#else
  ...
  // Restore R3 to R11 from the stack
  asm("ldm  r0!, {r3-r11}");
  asm("mov lr, r3");
#endif
#elif defined(YSS__CORE_CM0_H_GENERIC)
    ...
    // Initialize the SysTick counter.
    // Since SysTick overflowed before context switching started,
    // the counter might have decremented if an interrupt occurred in between,
    // so reset it.
    asm("ldr r3, =0xe000e010");
    asm("movs r1, #0");
    asm("str r1, [r3, #8]");

    // Restore R8 to R11 from the stack
    asm("add r0, r0, #20");
    asm("ldm  r0!, {r3-r6}");
    asm("mov r8, r3");
    asm("mov r9, r4");
    ...
    // Restore R3 to R7 from the stack
    asm("sub r0, r0, #36");
    asm("ldm  r0!, {r3-r7}");
    asm("add r0, r0, #16");
#endif
  // Move the stack pointer stored in R0 to PSP
  asm("msr psp, r0");
#endif
  asm("bx lr");
}
